...
The main idea is that when we create React elements using jsx they don't
actually give us access to the DOM. The DOM nodes aren't created until React
renders it so our functional component is responsible for creating and return
react elements but nothing in the DOM.

When using external libraries we might need to manipulate the DOM so the
workaround for this is to use `React.useRef()` which allows us to give access to
a particular DOM node when it renders the component. This is done by using a
special prop called `ref`. After we render the component, its mounted and when
we use the `React.useEffect()` in tandem the ref should have its `current`
property set to the DOM node.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=05%3A%20useRef%20and%20useEffect%3A%20DOM%20interaction&em=kevin.d.kim1116%40gmail.com).
