...
Saving the state into local storage was a bit tricky because I realized the
concept was that the `squares` state would be what we were saving into local
storage but I made the mistake of using the localStorage api incorrectly. On
this snippet of JS:
...
I made several mistakes...the first being that you can't set anything into local
storage without doing a key value pair and I did json stringify or parse the
array. This is because in order to store the array as a value we need to
stringify to create a JSON string to insert into local storage. Afterwards, we
need to deserialize it with JSON.parse back to a javascript object.

```javascript
function Board() {
  const [squares, setSquares] = React.useState(
   () =>
   JSON.Parse(window.localStorage.getItem('squares', squares)) || Array(9).fill(null)
  )
...
Holy moly, this extra credit to create the tic-tac-toe board was extremely
difficult. The main idea that I struggled on was figuring out how to determine
what state we needed for the board and how to wire it up. One thing that needs
to be revisited is the process of figuring this out as its very methodical but I
approached this as trying to do it all in one bite.
